...
//    s_cpu *cpu = &emu->cpu;
//    
//    screen->LCD_PPU_enable          = cpu->io_reg.LCDC & 0x80;
//    screen->win_tile_map_area       = cpu->io_reg.LCDC & 0x40;
//    screen->window_enable           = cpu->io_reg.LCDC & 0x20;
//    screen->BG_win_tile_data_area   = cpu->io_reg.LCDC & 0x10;
//    screen->BG_tile_map_area        = cpu->io_reg.LCDC & 0x08;
//    screen->obj_size                = cpu->io_reg.LCDC & 0x04;
//    screen->obj_enable              = cpu->io_reg.LCDC & 0x02;
//    screen->bg_win_enable_priority  = cpu->io_reg.LCDC & 0x01;
//    
//    memset(screen->current_row, 0, sizeof(screen->current_row));
//    
////    if(cpu->io_reg.LY < 144)
////    {
////        if(0 != scan_OAM(emu))
////            return EXIT_FAILURE;
////        for(size_t i = 0; i < PIX_BY_W; i++)
////        {
////            screen->pixels[cpu->io_reg.LY * PIX_BY_W + i] = SDL_MapRGBA(
////                screen->format, 
////                85 * screen->current_row[i], 
////                85 * screen->current_row[i], 
////                85 * screen->current_row[i], 
////                255
////            );
...
//    uint16_t data_start_adress = screen->BG_win_tile_data_area ? 0x8000 : 0x8800;
//
//    if(cpu->io_reg.LY + cpu->io_reg.SCY > 255)
//    {
//        fprintf(stderr, "WARNING: LY + SCY > 255 !!\n");
//        return EXIT_FAILURE;
//    }
//    for(size_t i = 0; i < PIX_BY_W / 8; i++)
//    {
//        uint8_t tileno = cpu->VRAM[map_start_adress + (cpu->io_reg.LY + cpu->io_reg.SCY + cpu->io_reg.SCX) / 8 + i]
//        screen->current_row[i] = cpu->VRAM[]
//    }
//    
//    return EXIT_SUCCESS;
//}
