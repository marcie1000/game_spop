...
void NOP(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    emu->cpu.t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;
}
...
void LD_BC_d16(void *arg, uint32_t op)
{
    ...
    emu->cpu.t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //12;
}
...
void ADD_HL_BC(void *arg, uint32_t op)
{
    ...
    uint16_t BC12bits = BC & 0x0FFF;
    
    flag_assign(HL12bits + BC12bits > 0x0FFF, &cpu->regF, HALF_CARRY_FMASK);
    flag_assign(HL > UINT16_MAX - BC, &cpu->regF, CARRY_FMASK);
    flag_assign(false, &cpu->regF, NEGATIVE_FMASK);
...
void LD_A_derefBC(void *arg, uint32_t op)
{
    ...
    uint8_t data;
    if(0 != read_memory(emu, (cpu->regB << 8) + cpu->regC, &data))
        destroy_emulator(emu, EXIT_FAILURE);
    cpu->regA = data;
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //8;
}
...
void INC_C(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
...
void RRCA(void *arg, uint32_t op)
{
    ...
    flag_assign(false, &cpu->regF, NEGATIVE_FMASK | HALF_CARRY_FMASK | ZERO_FMASK);
    cpu->regA = newA;
...
void LD_DE_d16(void *arg, uint32_t op)
{
    ...
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //12;
}
...
void LD_A_derefDE(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
...
void JR_NZ_r8(void *arg, uint32_t op)
{
    ...
    if(!(cpu->regF & ZERO_FMASK)) //if Z flag is 0
    {
        ...
        cpu->pc += value;
        emu->cpu.t_cycles += emu->timing_table[1][(op & 0x00FF0000) >> 16]; //12;
    }
    else
        emu->cpu.t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //8;
}
...
void JR_Z_r8(void *arg, uint32_t op)
{
    ...
    if(cpu->regF & ZERO_FMASK)
    {
        int8_t r8 = (op & 0x0000ff00) >> 8;
        cpu->pc += r8;
        cpu->t_cycles += emu->timing_table[1][(op & 0x00FF0000) >> 16]; //12;
    }
    else
    {
        cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //8;
...
void LD_derefHLminus_A(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
    if(0 != write_memory(emu, (cpu->regH << 8) + cpu->regL, cpu->regA))
...
void ADD_HL_SP(void *arg, uint32_t op)
{
    ...
    uint16_t HL12bits = HL & 0x0FFF;
    uint16_t SP12bits = cpu->sp & 0x0FFF;
    
    flag_assign(HL12bits + SP12bits > 0x0FFF, &cpu->regF, HALF_CARRY_FMASK);
    flag_assign(HL > UINT16_MAX - cpu->sp, &cpu->regF, CARRY_FMASK);
    flag_assign(false, &cpu->regF, NEGATIVE_FMASK);
...
void CCF(void *arg, uint32_t op)
{
    ...
    flag_assign(!(cpu->regF & CARRY_FMASK), &cpu->regF, CARRY_FMASK);
    flag_assign(false, &cpu->regF, NEGATIVE_FMASK | HALF_CARRY_FMASK);
...
void LD_derefHL_A(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
    if(0 != write_memory(emu, (cpu->regH << 8) + cpu->regL, cpu->regA))
...
void LD_A_H(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
    cpu->regA = cpu->regH;
...
void ADC_A_derefHL(void *arg, uint32_t op)
{
    ...
    flag_assign(A4bit + data4bit + carry > 0x0F, &cpu->regF, HALF_CARRY_FMASK);
...
void SBC_A_B(void *arg, uint32_t op)
{
    ...
    flag_assign(B4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regB + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regB + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_C(void *arg, uint32_t op)
{
    ...
    flag_assign(C4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regC + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regC + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_D(void *arg, uint32_t op)
{
    ...
    flag_assign(D4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regD + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regD + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_E(void *arg, uint32_t op)
{
    ...
    flag_assign(E4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regE + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regE + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_H(void *arg, uint32_t op)
{
    ...
    flag_assign(H4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regH + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regH + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_L(void *arg, uint32_t op)
{
    ...
    flag_assign(L4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(cpu->regL + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= cpu->regL + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //4;   
}
void SBC_A_derefHL(void *arg, uint32_t op)
{
    ...
    flag_assign(data4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(data + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= data + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //8;   
...
void XOR_A(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
    cpu->regA ^= cpu->regA;
...
void JP_NZ_a16(void *arg, uint32_t op)
{
    ...
    cpu->pc -= 3;
    cpu->t_cycles += emu->timing_table[1][(op & 0x00FF0000) >> 16]; //16;
}
...
void CALL_NZ_a16(void *arg, uint32_t op)
{
    ...
    //take the pc incrementation in the interpret function into account
    cpu->pc -= 3;
    cpu->t_cycles += emu->timing_table[1][(op & 0x00FF0000) >> 16]; //24;    
}
...
void CALL_a16(void *arg, uint32_t op)
{
    ...
    //take the pc incrementation in the interpret function into account
    cpu->pc -= 3;
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //24;    
}
...
void SBC_A_d8(void *arg, uint32_t op)
{
    ...
    flag_assign(d8_4bit + cy > A4bit, &cpu->regF, HALF_CARRY_FMASK);
    
    flag_assign(d8 + cy > cpu->regA, &cpu->regF, CARRY_FMASK);
    flag_assign(true, &cpu->regF, NEGATIVE_FMASK);
    cpu->regA -= d8 + cy;
    flag_assign(cpu->regA == 0, &cpu->regF, ZERO_FMASK);
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //8;   
...
void LD_derefC_A(void *arg, uint32_t op)
{
    s_emu *emu = arg;
    s_cpu *cpu = &emu->cpu;
...
void ADD_SP_r8(void *arg, uint32_t op)
{
    ...
    uint8_t r8u4bit = ((op & 0x00000F00) >> 8);
    uint8_t r8u = ((op & 0x0000FF00) >> 8);
    
    uint8_t SP8bit = cpu->sp & 0xFF;
    
    flag_assign(false, &cpu->regF, ZERO_FMASK | NEGATIVE_FMASK);
    flag_assign(SP4bit + r8u4bit > 0x0f, &cpu->regF, HALF_CARRY_FMASK);
    flag_assign(SP8bit > UINT8_MAX - r8u, &cpu->regF, CARRY_FMASK);
    
    cpu->sp += r8;
    cpu->t_cycles += emu->timing_table[0][(op & 0x00FF0000) >> 16]; //16;
    
//    if(sp_init == 0 && (cpu->regF & HALF_CARRY_FMASK))
//    {
//        printf("%lu\n", cpu->inst_counter);
//        printf("");
//    }
...
void LD_A_derefC(void *arg, uint32_t op)
{
    ...
    if(0 != read_memory(emu, 0xFF00 + cpu->regC, &cpu->regA))
        destroy_emulator(emu, EXIT_FAILURE);
